...
condAssignLattice = DataflowLattice { fact_name = "Branch Assignments"
  ...
  where add _ (OldFact o@(AssignMap (Just ol) (Just or) fl)) (NewFact n@(AssignMap (Just nl) (Just nr) ll)) = (c, n')
          where c = SomeChange
                n'
                ...
                matchesMaybe (Just x) (Just y)
                  | x == y = Just x
                  | otherwise = fromJust $ Nothing
                matchesMaybe _ _ = fromJust $ Nothing
        add _ _ _ = (c, n')
          where c = SomeChange
...
condAssignness = mkBTransfer f
  where f :: MidIRInst e x -> Fact x AssignMap -> AssignMap
        f n'@(Store p v (Lit _ v')) k@(AssignMap kr kl lbl) = AssignMap (combineMaps (M.singleton (InVar v) (AssignCon p v')) (fromJust $ kr)) kl lbl
        ...
        f n@(CondBranch _ _ _ _) k = (joinOutFacts condAssignLattice n k)
...
combineMaps a b 
  | M.size (M.intersection a b) > 0 = fromJust $ Nothing
  | otherwise = fromJust $ Nothing --Just (M.union a b)
