...
lastReturnTransfer = mkBTransfer f
    where f :: MidIRInst e x -> Fact x LastReturn -> LastReturn
          ...
          f (ThreadReturn _ l) k = fromMaybe RUnknown $ lookupFact l k
          f (Parallel _ ll _ _ el) k = combine elf llf
            where elf = fromMaybe RUnknown $ lookupFact el k
                  llf = fromMaybe RUnknown $ lookupFact ll k
