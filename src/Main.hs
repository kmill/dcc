...
main = do args <- getArgs
          ...
          case target opts of
            ...
            TargetDefault -> case lowir of
              Left err -> do (putStrLn err)
                             exitWith $ ExitFailure 1
              Right lir -> do writeFile outFile (unlines $ CodeGenerate2.lowIRToAsm lir)
                              if debugMode opts then putStrLn $ intercalate "\n" (CodeGenerate2.lowIRToAsm lir) else return ()                              
...
doLowIRFile opts ifname input midir
    = case midir of                      
        ...
        Right m -> let assem = do a <- CodeGenerate2.toAss m
                                  a <- if debugMode opts then return a else  RegisterAllocator2.regAlloc a
                                  return a
