...
instance ShowC String where
    showC = id

data ExprWrap v = EW (Expr v)
instance (Show v, ShowC v) => Show (ExprWrap v) where
    showsPrec _ (EW (Lit pos x)) = shows x
    showsPrec _ (EW (LitLabel pos lab)) = showString lab
    showsPrec _ (EW (Var pos v)) = (++) (showC v)
    showsPrec _ (EW (Load pos expr)) = showString "*(" . showsPrec 0 expr . showString ")"
    showsPrec p (EW (UnOp pos op expr)) = showParen (p>0) (shows op . showString " " . showsPrec 1 expr)
    showsPrec p (EW (BinOp pos op ex1 ex2))
        = showParen (p>0) (showsPrec 1 ex1 . showString " " . shows op . showString " " . showsPrec 1 ex2)
    showsPrec p (EW (Cond pos exc ex1 ex2))
        = showParen (p>0) (showsPrec 1 exc
                           . showString " ? " . showsPrec 1 ex1
                           . showString " : " . showsPrec 1 ex2)

instance (Show v, ShowC v) => ShowC (Expr v) where
    showC = show . EW

showT :: (Show a) => a -> String
showT = tail . show

instance (Show v, ShowC v) => ShowC (Inst v e x) where
    showC (Label pos lbl)
        = printf "%s: // {%s}"
          (showC lbl) (showPos pos)
    showC (Enter pos lbl args)
        = printf "int %s; // {%s}"
          (intercalate ", " (map showC args)) (showPos pos)
    showC (Store pos var expr)
        = printf "int %s = %s; // {%s}"
          (showC var) (showC expr) (showPos pos)
    showC (IndStore pos dest expr)
        = printf "*(%s) = %s; // {%s}"
          (showC dest) (showC expr) (showPos pos)
    showC (Call pos dest name args)
        = printf "int %s = method_%s(%s); // {%s}"
          (showC dest) name (intercalate ", " $ map showC args) (showPos pos)
    showC (Callout pos dest name args)
        = printf "int %s = %s(%s); // {%s}"
          (showC dest) name (intercalate ", " $ map showC args) (showPos pos)
    showC (Branch pos lbl)
        = printf "goto %s; // {%s}"
          (showC lbl) (showPos pos)
    showC (CondBranch pos expr tlbl flbl)
        = printf "if (%s) // {%s}\n  goto %s;\nelse\n  goto %s;"
          (showC expr) (showPos pos) (showC tlbl) (showC flbl)
    showC (Return pos mexpr)
        = printf "return %s; // {%s}"
          (maybe "" showC mexpr) (showPos pos)
    showC (Fail pos)
        = printf "fail; // {%s}"
          (showPos pos)

midIRToC m = "void main()\n{\n"
             ++ (showFields (midIRFields m))
             ++ (showStrings (midIRStrings m))
             ++ (showMethods (midIRMethods m))
             ++ graphToC showC (midIRGraph m)
             ++ "}"
...
graphToC node (GMany _ g_blocks _) = concatMap bviz (mapElems g_blocks)
  where bviz block = lab ++ ": \n"
            ...
            where lab = showC $ entryLabel block
                  showEdge e x = printf "%" (show e) (show x)
        b block = node a ++ "\n" ++ unlines (map node bs) ++ node c ++ "\n"
            where f :: (MaybeC C (n C O), [n O O], MaybeC C (n O C))
                    -> (n C O, [n O O], n O C)
                  f (JustC e, nodes, JustC x) = (e, nodes, x)
